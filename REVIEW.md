...
- [x] Temperature: 1 decimal place string, retain true, QoS 0
  `src/main.cpp:368–370` — `format_float_1dp(temp, ...)` then `mqtt_publish_measurement(...)`
...
- [x] Humidity: 1 decimal place string, retain true, QoS 0
  `src/main.cpp:372–374` — same pattern as temperature
...
## Iteration 2 — Battery Monitoring Compliance Review

Generated by Agent 12. Every `[x]` entry cites the actual file and line number verified by
reading the source. Every `[ ] DEVIATION` entry deviates from the requirement.

---

### IT2-1. Battery Voltage ADC Read

- [x] ADC scale constant defined (`BATTERY_ADC_SCALE`)
  `src/main.cpp:16` — `#define BATTERY_ADC_SCALE (4.2f / 1023.0f)` — Wemos D1 Mini Battery Shield v1.1.0 ratio

- [x] Battery voltage read from ADC at A0 each cycle
  `src/main.cpp:297` — `int adc_raw = analogRead(A0)`
  `src/main.cpp:298` — `float battery_v = (float)adc_raw * BATTERY_ADC_SCALE`
  `src/main.cpp:299` — `Serial.printf("[Batt] ADC raw=%d  voltage=%.2fV\n", adc_raw, battery_v)`

- [x] Battery voltage published to `{topic_root}/esp-{chip_id}/telemetry/voltage` each cycle
  `src/main.cpp:309` — `build_telemetry_topic(cfg.mqtt_topic_root, device_name, "voltage", topic_volt, sizeof(topic_volt))`
  `src/main.cpp:377–383` — Step 9b: voltage publish block — always published (not gated on sensor_ok)
  `src/main.cpp:381` — `mqtt_publish_measurement(mqtt_client, topic_volt, volt_buf)`

---

### IT2-2. Telemetry Topic Hierarchy

- [x] `build_telemetry_topic()` helper inserts `/telemetry/` segment between device and subtopic
  `src/utils.h:25–28` — `snprintf(buf, len, "%s/%s/telemetry/%s", root, device, sub)`

- [x] Temperature topic uses `build_telemetry_topic()` (telemetry namespace)
  `src/main.cpp:307` — `build_telemetry_topic(cfg.mqtt_topic_root, device_name, "temperature", topic_temp, sizeof(topic_temp))`

- [x] Humidity topic uses `build_telemetry_topic()` (telemetry namespace)
  `src/main.cpp:308` — `build_telemetry_topic(cfg.mqtt_topic_root, device_name, "humidity", topic_hum, sizeof(topic_hum))`

- [x] Voltage topic uses `build_telemetry_topic()` (telemetry namespace)
  `src/main.cpp:309` — `build_telemetry_topic(cfg.mqtt_topic_root, device_name, "voltage", topic_volt, sizeof(topic_volt))`

- [x] Status topic uses `build_topic()` (unchanged from Iteration 1 — no telemetry segment)
  `src/main.cpp:306` — `build_topic(cfg.mqtt_topic_root, device_name, "status", topic_status, sizeof(topic_status))`
  `src/utils.h:16–19` — `snprintf(buf, len, "%s/%s/%s", root, device, sub)` — no telemetry infix

---

### IT2-3. Status Payload — Battery Priority

- [x] `battery_status_str()` helper: returns `"BAT_CRIT"` at or below critical_v, `"BAT_LOW"` at or below low_v, `nullptr` otherwise
  `src/utils.h:31–35` — `if (battery_v <= critical_v) return "BAT_CRIT"; if (battery_v <= low_v) return "BAT_LOW"; return nullptr`

- [x] Status publish uses battery status string when present; falls back to `"OK"` / `"NOK"` from sensor state
  `src/main.cpp:359` — `const char* batt_str = battery_status_str(battery_v, cfg.battery_low_v, cfg.battery_critical_v)`
  `src/main.cpp:360` — `const char* status_str = batt_str ? batt_str : (sensor_ok ? "OK" : "NOK")`

- [x] Priority order: BAT_CRIT > BAT_LOW > OK / NOK — battery state always overrides sensor state
  `src/utils.h:32–34` — critical checked first (`<=`), then low (`<=`); `battery_status_str` returns on first match
  `src/main.cpp:360` — ternary: if `batt_str` is non-null it is used regardless of `sensor_ok`

---

### IT2-4. Battery-Based Sleep Selection

- [x] Sleep duration selected top-to-bottom, first match wins: critical_v → critical_battery_s; low_v → low_battery_s; else normal_s
  `src/main.cpp:390–396` — `if (battery_v <= cfg.battery_critical_v) sleep_s = cfg.sleep_critical_battery_s; else if (battery_v <= cfg.battery_low_v) sleep_s = cfg.sleep_low_battery_s; else sleep_s = cfg.sleep_normal_s`

- [x] Battery thresholds read from config (`battery.low_v`, `battery.critical_v`)
  `src/main.cpp:391` — `cfg.battery_critical_v` sourced from `config_load` → `ConfigManager.cpp:88`
  `src/main.cpp:393` — `cfg.battery_low_v` sourced from `config_load` → `ConfigManager.cpp:86`

- [x] Battery sleep durations read from config (`sleep.low_battery_s`, `sleep.critical_battery_s`)
  `src/main.cpp:392` — `cfg.sleep_critical_battery_s` sourced from `config_load` → `ConfigManager.cpp:80`
  `src/main.cpp:394` — `cfg.sleep_low_battery_s` sourced from `config_load` → `ConfigManager.cpp:78`

---

### IT2-5. Chained Sleep Implementation

- [x] `SLEEP_MAGIC` sentinel constant defined
  `src/main.cpp:17` — `#define SLEEP_MAGIC 0xDEADBEEF`

- [x] `SLEEP_MAX_S` hardware limit constant defined (`4294`)
  `src/main.cpp:18` — `#define SLEEP_MAX_S 4294`

- [x] `sleep_chained()` function: persists remaining duration in RTC user memory (offset 0, 8 bytes)
  `src/main.cpp:26–36` — writes `rtc[0] = SLEEP_MAGIC` (if remaining > 0) and `rtc[1] = remaining` via `ESP.rtcUserMemoryWrite(0, rtc, sizeof(rtc))`

- [x] `sleep_chained()` sleeps in at most `SLEEP_MAX_S`-second segments
  `src/main.cpp:28` — `uint32_t chunk = ((uint32_t)total_s > SLEEP_MAX_S) ? SLEEP_MAX_S : (uint32_t)total_s`

- [x] `sleep_chained()` calls `led_off()` before deep sleep
  `src/main.cpp:34` — `led_off()` called before `ESP.deepSleep()`

- [x] `sleep_chained()` uses `uint64_t` cast on sleep duration to prevent silent overflow
  `src/main.cpp:35` — `ESP.deepSleep((uint64_t)chunk * 1000000ULL)`

- [x] Sleep called via `sleep_chained()` — handles any duration including those above 4294s
  `src/main.cpp:399` — `sleep_chained(sleep_s)` — replaces direct `ESP.deepSleep()` for the normal sleep path at end of cycle

---

### IT2-6. Chained Sleep Continuation Check

- [x] On wake, firmware checks RTC user memory for a pending chained sleep segment before running the full publish cycle
  `src/main.cpp:154–173` — block at the top of `setup()`, immediately after `led_init()`

- [x] Magic sentinel `SLEEP_MAGIC` validated; remaining seconds checked > 0
  `src/main.cpp:157` — `if (rtc[0] == SLEEP_MAGIC && rtc[1] > 0)`

- [x] Continuation: sleeps next chunk, updates RTC remainder, does not run publish cycle
  `src/main.cpp:158–163` — reads remaining, computes next chunk, writes updated rtc, calls `ESP.deepSleep()`

- [x] Non-continuation: clears RTC state and falls through to full publish cycle
  `src/main.cpp:170–172` — `rtc[0] = 0; rtc[1] = 0; ESP.rtcUserMemoryWrite(0, rtc, sizeof(rtc))`

---

### IT2-7. Unit Tests — New Iteration 2 Utilities

All 6 new tests are in `test/test_all.cpp` and are run under `pio test -e native`. Tests 1–8 are Iteration 1 tests; tests 9–14 are the new Iteration 2 additions.

- [x] Test 9 — `test_build_telemetry_topic_standard`: standard root → `devices/esp-a1b2c3/telemetry/temperature`
  `test/test_all.cpp:98–102`

- [x] Test 10 — `test_build_telemetry_topic_nested_root`: nested root → `home/env/esp-a1b2c3/telemetry/humidity`
  `test/test_all.cpp:104–108`

- [x] Test 11 — `test_battery_status_str_critical`: voltage 3.1V (below 3.2 critical) → `"BAT_CRIT"`
  `test/test_all.cpp:112–114`

- [x] Test 12 — `test_battery_status_str_low`: voltage 3.3V (above 3.2 critical, below 3.5 low) → `"BAT_LOW"`
  `test/test_all.cpp:116–118`

- [x] Test 13 — `test_battery_status_str_normal`: voltage 3.8V (above both thresholds) → `nullptr`
  `test/test_all.cpp:120–122`

- [x] Test 14 — `test_battery_status_str_boundary_crit`: voltage exactly at 3.2 critical → `"BAT_CRIT"` (boundary uses `<=`)
  `test/test_all.cpp:124–127`

---

### IT2-8. Deviations and Notable Findings

#### DEVIATION IT2-1 — Battery Voltage and Telemetry Publish QoS: 0 Instead of 1

**Requirement (REQUIREMENTS.md §Iteration 2):** All publish calls in Iteration 2 (including voltage) follow the same QoS rules as Iteration 1 status: retain=true. Iteration 1 Deviation 2 (§10) already documented that `mqtt_publish_status` uses QoS 0 due to PubSubClient's `publish(topic, payload, retained)` overload not accepting a QoS argument.

**Actual (`lib/MqttClient/MqttClient.cpp:41`):** `mqtt_publish_measurement` uses `client.publish(topic, payload, true)` — QoS 0, retain=true. The voltage publish at `src/main.cpp:381` calls `mqtt_publish_measurement`, so voltage is also QoS 0 with retain=true.
...
### IT2-9. Iteration 2 Verification Coverage

| Requirement Area | Verification Method |
| --- | --- |
| `BATTERY_ADC_SCALE` constant | Build-verified (`src/main.cpp:16`) |
| `SLEEP_MAGIC` constant | Build-verified (`src/main.cpp:17`) |
| `SLEEP_MAX_S` constant | Build-verified (`src/main.cpp:18`) |
| `sleep_chained()` segments correctly | Code review (`src/main.cpp:26–36`) |
| Chained sleep continuation check | Code review (`src/main.cpp:154–173`) |
| Battery ADC read (`analogRead(A0)`) | Build-verified (`src/main.cpp:297–299`) |
| `build_telemetry_topic()` helper | Unit test (`test/test_all.cpp:98–108`) — tests 9–10 |
| `battery_status_str()` helper | Unit test (`test/test_all.cpp:112–127`) — tests 11–14 |
| Telemetry topic for temperature | Build-verified (`src/main.cpp:307`) |
| Telemetry topic for humidity | Build-verified (`src/main.cpp:308`) |
| Telemetry topic for voltage | Build-verified (`src/main.cpp:309`) |
| Status topic unchanged (no telemetry infix) | Build-verified (`src/main.cpp:306`) |
| Battery status priority (BAT_CRIT/BAT_LOW/OK/NOK) | Code review (`src/main.cpp:359–360`) |
